...
def find_data_days(symbol, interval_s, days):
    ...
    fig.savefig('newfile.png', bbox_inches='tight', dpi=300)
...
def data_plot():
    ...
    while True:
        period = input('Define how long you want to see prices for:\nEnter M for Months\nEnter Y for Years\nEnter D for Days\n')
        if period.upper() == 'M':
            time_range = input('Enter number of months you want to see:\n')
            find_data_mny(str(symbol), int(time_range))
            break
        elif period.upper() == 'Y':
            time_range = input('Enter number of years you want to see:\n')
            find_data_mny(str(symbol), int(time_range) * 12)
            break
        elif period.upper() == 'D':
            time_range = input('Enter 1 or 5, if you enter anything else it will default to 5 days:\n')
            find_data_days(str(symbol), 60, int(time_range))
            break
        else:
            print('Invalid Input!\n Enter M, Y, or D\n')
